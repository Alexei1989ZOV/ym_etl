...
class PricesCSVTransformer(BaseCSVTransformer):
    """
    Трансформер для отчета по ценам.
    Приводит данные из CSV в необходимые типы для загрузки в БД.
    """
    def __init__(self, csv_path):
        """
        Args:
            csv_path: Путь к CSV-файлу с отчетом по ценам.
        """
    ...
    def _extract_date(self):
        ...
        try:
            ...
            logger.debug(f"[prices] Извлечена дата из имени CSV: {date}")
            return date
        except:
            logger.error(f"[prices] Не удалось извлечь дату из имени файла: {name}")
