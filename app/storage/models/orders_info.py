...
class OrdersStatusesTbl(Base):
    ...
    order_id: Mapped[int] = mapped_column(BIGINT, ForeignKey("orders.orders.order_id", ondelete="CASCADE"), primary_key=True)
    order_status: Mapped[str] = mapped_column(String, nullable=False)
    date_from: Mapped[datetime] = mapped_column(TIMESTAMP, nullable=True, primary_key=True)
    date_to: Mapped[datetime] = mapped_column(TIMESTAMP, nullable=True)
...
class OrdersCommissionsTbl(Base):
    ...
    commission_amount: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=False)
    report_date: Mapped[date] = mapped_column(Date, nullable=False, default=date.today())
...
class OrderItemsTbl(Base):
    ...
    order_id: Mapped[int] = mapped_column(BIGINT, ForeignKey("orders.orders.order_id", ondelete="CASCADE"), primary_key=True)
    shop_sku: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    seller_price_per_unit: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=False)
    buyer_price_per_unit: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=False)
    mp_discount_per_unit: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=True)
    mp_yandex_plus_discount: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=True)
...
class OrderPaymentsTbl(Base):
    ...
    order_id: Mapped[int] = mapped_column(BIGINT, ForeignKey("orders.orders.order_id", ondelete="CASCADE"),
                                          primary_key=True)
    payment_id: Mapped[int] = mapped_column(BIGINT, primary_key=True)
    ...
    payment_amount: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=False)
...
class OrdersSubsidiesTbl(Base):
    ...
    subsidy_amount: Mapped[Decimal] = mapped_column(DECIMAL(9, 2), nullable=False)
    report_date: Mapped[date] = mapped_column(Date, nullable=False, default=date.today())